...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
    // TODO: IMPLEMENT
    volatile u16 *colorPointer = &(color);
    DMA[3].cnt = 0;
    for (int i = 0; i < height; i++) {
        volatile unsigned short *rectDest = videoBuffer + OFFSET(row + i, col, WIDTH);
        DMA[3].dst = rectDest;
        DMA[3].src = colorPointer;
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
    ...
    DMA[3].cnt = 0;
    for (int i = 0; i < height; i++) {
        volatile unsigned short *imageDest = videoBuffer + OFFSET(row + i, col, WIDTH);
        DMA[3].dst = imageDest;
...
void fillScreenDMA(volatile u16 color) {
    DMA[3].src = &color; // Source address points to the color value
    DMA[3].dst = videoBuffer; // Destination address is the start of the video buffer
